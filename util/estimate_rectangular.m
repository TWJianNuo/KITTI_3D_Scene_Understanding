function [params, cuboid, val] = estimate_rectangular(pts_3d)
    pts_2d = pts_3d(:, 1:2);
    ...
    dist_record_ = zeros(length(theta_stack), 1);
    ...
    for i = 1 : length(theta_stack)
        ...
        [sum_dist, all_dist] = calculate_sum_dist(pts_2d, rectangle);
        dist_record(i) = sum_dist; dist_record_(i) = all_dist;
        rectangles_record{i} = rectangle;
    end
    [~, ind] = min(dist_record); val = dist_record_(ind);
    ...
    cuboid = generate_cuboid_by_center_(cx, cy, theta, l, w, h);
end
...
function [sum_dist, all_dist] = calculate_sum_dist(pts_2d, rectangle)
    pts_2d = [pts_2d ones(size(pts_2d, 1), 1)];
    lines = rectangle.lines;
    A1 = [1 0 0]; A2 = [0 1 0];
    dist_for_four = abs((lines * pts_2d')') ./ repmat(sqrt((A1 * lines').^2 + (A2 * lines').^2), [size(pts_2d, 1), 1]);
    val = sort(min(dist_for_four,[],2));
    sum_dist = sum(val(1:ceil(end / 2))); all_dist = sum(val);
end