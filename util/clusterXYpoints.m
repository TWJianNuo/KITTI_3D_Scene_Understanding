...
%
%% Code
function clusters = clusterXYpoints(inputfile,maxdist,minClusterSize,method,mergeflag)
...
dim = size(inputfile,2);
XY = inputfile;
D = pdist(XY,'euclidean'); % compute pairwise distances between all points

kmax = size(XY,1);
pairs = zeros((1 + kmax-1) * (kmax - 1) / 2, 2); % column 1 = point1, column2 = point2
pair_count = 1;
for k = 2:kmax
    pairs(pair_count : pair_count + kmax-k, :) = [ (k:kmax)' , (k-1) * ones(kmax-(k-1),1) ];
    pair_count = pair_count + kmax-k+1;
end
...
clustersCentroids = NaN(clusterCount,dim);
for k=1:clusterCount
...
clustersGeoMedians = NaN(clusterCount,dim);
for k=1:clusterCount
